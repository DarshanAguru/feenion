...
def instrument_gemini(target: Any = None, tracer: Any = None) -> Any:
    """
    Instruments Google Gemini SDK clients or GenerativeModel instances to capture LLM calls
    automatically with model tracking, token metrics, latency, and cost calculation.
    ...
    def wrap_google_genai_generate(original_func: Any, default_model: str = "gemini-1.5-flash") -> Any:
    ...
    if target is not None:
        ...
        # Case 2: google-generativeai GenerativeModel instance
            ...
            model_name = getattr(target, "model_name", "gemini-1.5-flash")
        ...
        # Case 3: google-generativeai ChatSession instance
            ...
            model_name = getattr(getattr(target, "model", None), "model_name", "gemini-1.5-flash")
            target.send_message = wrap_google_genai_generate(target.send_message, default_model=model_name)

    return target

def wrap_gemini(client_or_model: Any) -> Any:
    """
    Convenience functional wrapper that instruments and returns the Gemini client or model.
    Example:
    `client = wrap_gemini(genai.Client())` or `model = wrap_gemini(genai.GenerativeModel('gemini-1.5-pro'))`
    """
    return instrument_gemini(client_or_model)
